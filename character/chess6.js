function chess6(x, y) {//红王
    ...
    chess.skill2_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill2_cooling <= 0) {
            ...
            for (var i = 0; i < num; i++) {
                document.getElementById(target[i]).hitfunction(target[i], -2, 0, selectid, chess.x, chess.y)
            }
    ...
    chess.skill3_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill3_cooling <= 0) {
            ...
            var bool = 0;//是否撞墙 
            target.hitfunction(input1, -1, 0, selectid, chess.x, chess.y);
            ...
            function skill3_2() {
                if (chess.x > target.x && detect_resist(target.x - 1, target.y) == 1 && bool == 0) { target.hitfunction(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.x < target.x && detect_resist(target.x + 1, target.y) == 1 && bool == 0) { target.hitfunction(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.y > target.y && detect_resist(target.x, target.y - 1) == 1 && bool == 0) { target.hitfunction(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.y < target.y && detect_resist(target.x, target.y + 1) == 1 && bool == 0) { target.hitfunction(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
            }
