function chess6(x, y) {//红王
    ...
    skill1.innerHTML = '选择最多5个目标<img height="25px" width="25px" src="./img/chooseA.png" id="input6_1_1" onclick="input(1,id,7)"><img height="25px" width="25px" src="./img/chooseA.png" id="input6_1_2" onclick="input(1,id,7)"><img height="25px" width="25px" src="./img/chooseA.png" id="input6_1_3" onclick="input(1,id,7)"><img height="25px" width="25px" src="./img/chooseA.png" id="input6_1_4" onclick="input(1,id,7)"><img height="25px" width="25px" src="./img/chooseA.png" id="input6_1_5" onclick="input(1,id,7)">拉动2格<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill1_launch()"></button>';
    ...
    skill3.innerHTML = '选择一个目标<img height="25px" width="25px" src="./img/chooseC.png" id="input6_3_1" onclick="input(1,id,1)">造成1点伤害并将其推动2格。若推动过程中或结束后目标碰到阻挡物则再造成一点伤害；否则技能冷却减一<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill3_launch()"></button>';
    chess.skill1_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill1_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            ...
            setTimeout(function () { selector(selectid, 1); skill(1); }, 10)
            overall_skill();
    ...
    chess.skill2_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill2_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            ...
            for (var i = 0; i < board.length; i++) {
                var tar = board[i];
                if (chess.direction == 1 && tar.class != 0 && ((tar.x == chess.x && tar.y == chess.y + 1) || (tar.x == chess.x && tar.y == chess.y + 2) || (tar.x == chess.x + 1 && tar.y == chess.y + 1) || (tar.x == chess.x - 1 && tar.y == chess.y + 1) || (tar.x == chess.x + 2 && tar.y == chess.y + 1) || (tar.x == chess.x - 2 && tar.y == chess.y + 1) || (tar.x == chess.x + 1 && tar.y == chess.y + 2) || (tar.x == chess.x - 1 && tar.y == chess.y + 2))) { target[num] = tar.id; num++; }
                if (chess.direction == 2 && tar.class != 0 && ((tar.x == chess.x + 1 && tar.y == chess.y) || (tar.x == chess.x + 1 && tar.y == chess.y + 1) || (tar.x == chess.x + 1 && tar.y == chess.y - 1) || (tar.x == chess.x + 1 && tar.y == chess.y + 2) || (tar.x == chess.x + 1 && tar.y == chess.y - 2) || (tar.x == chess.x + 2 && tar.y == chess.y + 1) || (tar.x == chess.x + 2 && tar.y == chess.y) || (tar.x == chess.x + 2 && tar.y == chess.y - 1))) { target[num] = tar.id; num++; }
                if (chess.direction == 3 && tar.class != 0 && ((tar.x == chess.x && tar.y == chess.y - 1) || (tar.x == chess.x && tar.y == chess.y - 2) || (tar.x == chess.x + 1 && tar.y == chess.y - 1) || (tar.x == chess.x - 1 && tar.y == chess.y - 1) || (tar.x == chess.x + 2 && tar.y == chess.y - 1) || (tar.x == chess.x - 2 && tar.y == chess.y - 1) || (tar.x == chess.x + 1 && tar.y == chess.y - 2) || (tar.x == chess.x - 1 && tar.y == chess.y - 2))) { target[num] = tar.id; num++; }
                if (chess.direction == 4 && tar.class != 0 && ((tar.x == chess.x - 1 && tar.y == chess.y) || (tar.x == chess.x - 1 && tar.y == chess.y + 1) || (tar.x == chess.x - 1 && tar.y == chess.y - 1) || (tar.x == chess.x - 1 && tar.y == chess.y + 2) || (tar.x == chess.x - 1 && tar.y == chess.y - 2) || (tar.x == chess.x - 2 && tar.y == chess.y + 1) || (tar.x == chess.x - 2 && tar.y == chess.y) || (tar.x == chess.x - 2 && tar.y == chess.y - 1))) { target[num] = tar.id; num++; }
            }
            for (var i = 0; i < num; i++) {
                hit(target[i], -2, 0, selectid, chess.x, chess.y)
            }
    ...
    chess.skill3_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill3_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            ...
            function skill3_2() {
                if (chess.x > target.x && detect_resist(target.x - 1, target.y) == 1 && bool == 0) { hit(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.x < target.x && detect_resist(target.x + 1, target.y) == 1 && bool == 0) { hit(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.y > target.y && detect_resist(target.x, target.y - 1) == 1 && bool == 0) { hit(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
                if (chess.y < target.y && detect_resist(target.x, target.y + 1) == 1 && bool == 0) { hit(input1, -1, 0, selectid, chess.x, chess.y); bool = 1; }
            }
            chess.skill3_cooling = chess.skill3_max_cooling;
            setTimeout(function () { if (bool == 0) { chess.skill3_cooling -= 1; } selector(selectid, 1); skill(3); }, 350)
            overall_skill();
        } else if (chess.fixedid != active_fixedid) {
            alert('无法行动：非此角色的行动回合');
    ...
    chess.effect[0] = 1;
    chess.range_display_array = ['A7', 'B17', 'C1'];//选择人物时范围演示
    chess.range_display_arrayA = [];//选择技能时范围演示
    chess.range_display_arrayB = ['B17'];//选择技能时范围演示
