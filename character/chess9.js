function chess9(x, y) {//定神游
    ...
    chess.skill2_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill2_cooling <= 0) {
            ...
            if (chess.data[1] == 0) {//非神游
                document.getElementById(input1).hitfunction(input1, -1, 0, selectid, chess.x, chess.y);
                for (var i = 1; i <= 100; i++) {
                ...
                if (bool == 1) {
                    ...
                    for (var i = 0; i < num; i++) {
                        document.getElementById(target[i]).hitfunction(target[i], -1, 0, selectid, chess.x, chess.y);
                    }
                    chess.skill2_cooling = chess.skill2_max_cooling;
                    selector(selectid, 1); skill(2);
                    overall_skill();
        ...
        if (chess.fixedid == active_fixedid && chess.skill3_cooling <= 0) {
            ...
            if (chess.data[1] == 0) {//非神游
                ...
                for (var i = 0; i < num; i += 2) {
                    if (target[i + 1] == 1) {
                        document.getElementById(target[i]).hitfunction(target[i], -3, 0, selectid, chess.x, chess.y);
                    }
                    if (target[i + 1] == 0) {
                        document.getElementById(target[i]).hitfunction(target[i], -1, 0, selectid, chess.x, chess.y);
                    }
