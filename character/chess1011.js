function chess1011(x, y) {//水史莱姆
    ...
    //以下需改动
    chess.fixedid = 1011;//1~1000人物1001~2000召唤物2001~3000地块
    chess.img = "./img/chess1011.png";
    chess.name = "水史莱姆";
    chess.max_health = 3;//1~30 20
    chess.health = 3;
    chess.max_movement = 3;//1~20 5
    chess.movement = 3;
    chess.reflect = 5;//1~20 5   -1不可行动
    skill1.innerHTML = '选择一个目标<img height="25px" width="25px" src="./img/chooseA.png" id="input1011_1_1" onclick="input(1,id,5)">造成一点伤害<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill1_launch()"></button>';
    ...
    chess.skill1_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill1_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            var input1 = document.getElementById("controller_page3").querySelector("#input1011_1_1").value;
            ...
            if (chess.effect_sum[2] == 0) {
                            ...
                            for (var j = 1; j <= 16; j++) { //找地
                                for (var k = 0; k < board.length; k++) {//找合适的输出位置
                                    if (board[k].x == i && board[k].y == j) {
                                        if (detect_range(5, board[h].id, board[k].id) == 1) {//改攻击范围   
                ...
                if (num == 0) {//在攻击范围内
                    if (chess.skill1_cooling == 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0)) {//能打
                        ...
                        setTimeout(function () { input(1, "input1011_1_1", 5) }, 400);//改三个数据
                                    ...
                                    for (var k = 0; k < board.length; k++) {
                                        if (board[k].x == i && board[k].y == j) {
                                            if (detect_range(5, board[h].id, board[k].id) == 1) {//改攻击范围   
                                                ...
                                                if (r[0] <= chess.movement && ((chess.data[2] == 1 && route(i, j, target.x, target.y, chess.max_movement, 1) < route(chess.x, chess.y, target.x, target.y, chess.max_movement, 1)) || (chess.data[2] == 2 && route(i, j, target.x, target.y, chess.max_movement, 1) > route(chess.x, chess.y, target.x, target.y, chess.max_movement, 1)))) {//更近或更远
                                                    dir = r[1];
                                                }
                                            }
                                            break;
                    ...
                    if (barrier == -1) { var move_bool = 1; }//走
                        ...
                        var move_bool = 1;
                        for (var i = 1; i < barrier[0]; i++) {
                            if (document.getElementById(barrier[i]).enemy == 0) {//有非友方障碍物
                                //判断能不能打到障碍物
                                var num = 9999;//所需步数
                                var dir = 0;//第一步方向 
                                for (var i = 1; i <= 16; i++) {//找地
                                    for (var j = 1; j <= 16; j++) { //找地
                                        for (var k = 0; k < board.length; k++) {//找合适的输出位置
                                            if (board[k].x == i && board[k].y == j) {
                                                if (detect_range(5, barrier[i], board[k].id) == 1) {//改攻击范围   
                                                    var r = route(chess.x, chess.y, board[k].x, board[k].y, chess.max_movement, 1)// 改每回合伤害
                                                    if (r[0] < num && r[2][0] == 1) {
                                                        num = r[0];
                                                        dir = r[1];
                                                        var target = document.getElementById(barrier[i]);
                                                    }
                                                }
                                                break;
                                            }
                    ...
                    if (move_bool == 1) {//走
                        ...
                        if (num == 0) {//在攻击范围内
                            if (chess.skill1_cooling == 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0)) {//能打
                                setTimeout(function () { skill(1); }, 200);
                                setTimeout(function () { input(1, "input1010_1_1", 1) }, 400);//改三个数据
                                setTimeout(function (x) { selector(x) }, 600, target.id);
                                setTimeout(function () { chess.skill1_launch() }, 800);
                                next = 1;
                            }
