function chess5(x, y) {//无人飞只因操作员
    ...
    chess.movement = 4;
    chess.reflect = 7;
    skill1.innerHTML = '选择位置<img height="25px" width="25px" id="input5_1_1" onclick="input(2,id,2)"><button style="position: absolute;right: 0px;bottom: 0px;height: 25px;width:25px;" onclick="document.getElementById(selectid).skill1_launch()"></button>';
    skill2.innerHTML = '选择位置<img height="25px" width="25px" id="input5_2_1" onclick="input(2,id,2)"><button style="position: absolute;right: 0px;bottom: 0px;height: 25px;width:25px;" onclick="document.getElementById(selectid).skill2_launch()"></button>';
    skill3.innerHTML = '所有无人机进入暴走状态，持续两回合。两回合后对所有无人机造成100点伤害<button style="position: absolute;right: 0px;bottom: 0px;height: 25px;width:25px;" onclick="document.getElementById(selectid).skill3_launch()"></button>';
    ...
    chess.skill3_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill3_cooling <= 0) {
            ...
            for (var i = 0; i < board.length; i++) {
                if ((board[i].fixedid == 1005 || board[i].fixedid == 1006) && board[i].class) {
                    board[i].data[1] = 1;//暴走
                }
            }
    ...
    chess.skill3_src = "./img/skill-1.png";//skill3
