function chess2(x, y) {//操作员
    ...
    //以下需改动
    chess.fixedid = 2;//1~1000人物1001~2000召唤物2001~3000地块
    chess.img = "./img/chess2.png";
    chess.name = "操作员";
    chess.max_health = 1;//1~30 20
    chess.health = 1;
    chess.max_movement = 50;//1~20 5
    chess.movement = 50;
    ...
    chess.skill1_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill1_cooling <= 0) {
            var input1 = document.getElementById("controller_page3").querySelector("#input2_1_1").value;
            chess1001(input1[0], input1[1]);
    ...
    chess.skill2_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill2_cooling <= 0) {
            var input1 = document.getElementById("controller_page3").querySelector("#input2_2_1").value;
            var input2 = document.getElementById("controller_page3").querySelector("#input2_2_2").value;
            var target = document.getElementById(input1);
            target.movefunction(input1, 5, [input2[0], input2[1]], 1, selectid, chess.x, chess.y);
    ...
    chess.effectfunction = function () { }
    chess.skillfunction = function (id) {//全局技能模块
        var chess = document.getElementById(id);
        if (round >= 1) {
            chess.hitfunction(id, -10, 0, id, chess.x, chess.y);
        }
    }
    ...
    chess.effect_sum = new Array(30);//1受伤改动2昏厥3束缚4泥沼5火毒6超重7攻击改动8反应改动9移动改动10最大生命改动11坚毅12大型13超重14免疫
    chess.effect[0] = 1;
    //tag
