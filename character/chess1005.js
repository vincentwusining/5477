function chess1005(x, y) {//战斗无人机
    ...
    //以下需改动
    chess.fixedid = 1005;//1~1000人物1001~2000召唤物2001~3000地块
    chess.img = "./img/chess1005.png";
    chess.name = "战斗无人机";
    chess.max_health = 3;
    chess.health = 3;
    ...
    chess.movement = 5;
    chess.reflect = 8;
    skill1.innerHTML = '选择一个目标<img height="25px" width="25px" id="input1005_1_1" onclick="input(1,id,2)">造成1点伤害。<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill1_launch()"></button>';
    ...
    chess.skill1_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill1_cooling <= 0) {
            ...
            if (chess.data[1] == 0) {
                document.getElementById(input1).hitfunction(input1, -1, 0, chess.id, chess.x, chess.y);
            }
            else {
                document.getElementById(input1).hitfunction(input1, -2, 0, chess.id, chess.x, chess.y);
            }
    ...
    chess.skillfunction = function (id) {
        ...
        if (chess.data[1] == 1 && chess.data[2] == 0) {
            chess.data[2] = 1;//已加速度
            chess.hitfunction(id, 0, 0, 3, 3, 0, 0, 0, id, chess.x, chess.y);
        }
    }
