function chess1(x, y) {//忍者
    ...
    chess.movement = 5;
    chess.reflect = 5;
    skill1.innerHTML = '选择周围四格内目标<img height="25px" width="25px" src="./img/chooseA.png" id="input1_1_1" onclick="input(1,id,1)">造成2点伤害<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill1_launch()"></button>';
    skill2.innerHTML = '选择曼哈顿距离小于7的目标<img height="25px" width="25px" src="./img/chooseB.png" id="input1_2_1" onclick="input(1,id,7)">造成1点伤害<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill2_launch()"></button>';
    skill3.innerHTML = '选择全屏目标<img height="25px" width="25px" src="./img/chooseC.png" id="input1_3_1" onclick="input(1,id,12)">(选填)将其推动3格。选择全屏目标<img height="25px" width="25px" src="./img/chooseC.png" id="input1_3_2" onclick="input(1,id,12)">(选填))将其拉动2格<button style="position: absolute;right: 0px;bottom: 0px;height: 50px;width:50px;" onclick="document.getElementById(selectid).skill3_launch()"></button>';
    chess.skill1_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill1_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            ...
            hit(input1, -2, 0, selectid, chess.x, chess.y);
    ...
    chess.skill2_launch = function () {
        ...
        if (chess.fixedid == active_fixedid && chess.skill2_cooling <= 0 && (chess.effect_sum[15] == 1 || chess.effect_sum[13] == 0) && chess.effect_sum[2] == 0) {
            ...
            var target = document.getElementById(input1);
            setTimeout(function () { hit(input1, -1, 0, selectid, chess.x, chess.y) }, 150)
    ...
    chess.effect[0] = 1;
    chess.range_display_array = ['A1', 'B7', 'C12'];//选择人物时范围演示
    chess.range_display_arrayA = [];//选择技能时范围演示
    chess.range_display_arrayB = [];//选择技能时范围演示
    chess.range_display_arrayC = [];//选择技能时范围演示//选择人物时范围演示
    chess.range_display_arrayA = [];//选择技能时范围演示
    chess.range_display_arrayB = [];//选择技能时范围演示
